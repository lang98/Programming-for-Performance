...
impl JwtSolver {
    ...
    fn check_parallel(&self) -> Option<Vec<u8>> {
        let (send_end, receive_end) = unbounded::<Vec<u8>>();
        let mut threads = vec![];
        // send
        threads.push(
            thread::scope(|s| {
                for &c in self.alphabet.iter() {
                    ...
                    s.spawn(move |_| {
                        let _ = match self.check_all([c].to_vec()) {
                            Some(result) => {
                                send_end.send(result).unwrap();
                            },
                            None => {}
                        };
                    });
        ...
        // receive
        let result = receive_end.recv().unwrap();
        for t in threads {
            drop(t);
        }
        return Some(result);
