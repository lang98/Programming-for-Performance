...
fn hackathon(args: &Args) {
    ...
    let (send_new, recv_new) = unbounded::<Event>();
    let (send_out, recv_out) = unbounded::<Event>();
    let (send_dl, recv_dl) = unbounded::<Event>();
    let mut threads = vec![];
    ...
    for i in 0..args.num_students {
        let mut student = Student::new(i, 
            Sender::clone(&send_new), Receiver::clone(&recv_new),
            Sender::clone(&send_out), Receiver::clone(&recv_out),
            Sender::clone(&send_dl), Receiver::clone(&recv_dl),
        );
    ...
    let mut start_idx = 0;
    let package_file = fs::read_to_string("data/packages.txt").unwrap();
    for i in 0..args.num_pkg_gen {
        let num_pkgs = per_thread_amount(i, args.num_pkgs, args.num_pkg_gen);
        let downloader = PackageDownloader::new(package_file.clone(), start_idx, num_pkgs, Sender::clone(&send_dl));
    ...
    let mut start_idx = 0;
    let product_file = fs::read_to_string("data/ideas-products.txt").unwrap();
    let customer_file = fs::read_to_string("data/ideas-customers.txt").unwrap();
    for i in 0..args.num_idea_gen {
        ...
        let generator = IdeaGenerator::new(
            ...
            num_students,
            num_pkgs,
            Sender::clone(&send_new),
            Sender::clone(&send_out),
        );
